...
import { deleteCosmoChange, updateCosmoChange } from "@/modules/marketing/service/cosmo-change-crud.service";
import { editCosmoChangeSchema } from "@/modules/marketing/validation";
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  const parsed = editCosmoChangeSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const result = await updateCosmoChange(id, parsed.data, {
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  ...
  const { id } = await params;
  const headerList = await headers();
  const result = await deleteCosmoChange(id, {
    userId: session.user.id,
    ipAddress: request.headers.get("x-forwarded-for") ?? request.headers.get("x-real-ip"),
    userAgent: headerList.get("user-agent"),
  });
